...
v1.20.0 highlights: **operator rating + exemplar-LoRA build loop + sigma/VAE knobs**. Closes the human-in-the-loop sprint (Phase 1 + Phase 2 + Phase 3 prereqs) plus operator-tunable stage 1 sigmas and VAE tiling controls. Schema v6 → v7 → v8 (additive ladder). 13 new endpoints across L1 (single-clip rating: `POST/PATCH/DELETE /v1/clips/{id}/rating[/{rid}]` + `GET /v1/clips/{id}/ratings`), L3 (queue + thresholds + boosts: `GET /v1/ratings/queue` cross-cohort diverse with `RATING_QUEUE_MIN_COHORT_DIVERSITY=0.3` default, `GET/POST /v1/api-keys/me/validator-thresholds` clamped to global ± 0.15, `session_boosts` param on `POST /v2/embeddings/recommend-loras`), and L2.5 (exemplar curation + LoRA build: `POST /v1/exemplar-sets`, list/detail/members CRUD, `POST /v1/exemplar-sets/{id}/build-lora` admin + async returning 202 in <500ms, `GET .../builds`, `POST .../build/{run_id}/cancel`). Path C′ writer lands `human_rating` preference_pairs with `signal_strength=0.75 + pending_construction_until=now+86400`; `scripts/construct_human_rating_pairs.py` runs hourly to re-validate 5 invariants and commit/delete; `train_dpo_sft.py` SELECT gains `AND pending_construction_until IS NULL`. New `scripts/build_lora_from_exemplars.py` (~360 LOC, dry-run by default; `--execute` spawns ltx-trainer subprocess with SIGTERM-safe cancellation) + `configs/exemplar_lora.yaml` (rank=32, steps=1500, lr=1e-4). 8 new error_codes plumbed through `_error()` envelope (EXEMPLAR_*, CLIP_*, RATE_LIMIT_TOO_FAST, SESSION_CAP_EXCEEDED); per-rater anti-fatigue 2-second min + 200/90min session cap; PATCH re-checks privacy gate (catches mid-session opt-in flip). Schema v8: `system_flags(name, value, updated_at)` for kill-switch + `preference_pairs.pending_construction_until` column; cross-cohort diversity in queue oversamples 4× and merges novel-cohort first. Sigma + VAE knobs (F-1/F-2/F-3): 7 new `gen_config` keys (`stage1_sigmas`, `pro_stage1_sigmas`, `vae_spatial_tile_px`, `vae_spatial_overlap_px`, `vae_temporal_tile_frames`, `vae_temporal_overlap_frames`, `vae_tiling_threshold_frames`); `_get_stage1_sigmas` helper centralizes 9 stage-1 sigma sites in `split_model_manager.py`; `_get_decode_tiling` reads 5 VAE keys; `POST /v1/system/config` returns structured 422 envelope (INVALID_SIGMA_LENGTH/RANGE/NAN/TYPE, INVALID_VAE_TILE_SIZE/OVERLAP/THRESHOLD) for hard violations + 200 with `warnings[]` for soft signals (non-monotonic, first<0.95, last>0.01); atomic config write via `.gen_config.json.tmp` → `os.replace` + append-only `.gen_config.history.jsonl` audit log; sidecar pass-through via `gen_config_overrides` body field on `ltx_sidecar_client.generate()` with 19-key whitelist forwarded by turbo dispatch. Dashboard adds Stage 1 Sigmas (Fast), Stage 1 Sigmas (Pro/HQ), VAE Tiling sections + 21 tooltips + default badges across every sampling/VAE knob. New `docs/RATING_LORA_INTEGRATION.md` (643 lines) — third-party client contract with curl per endpoint. Test suite 370 green / 0 fail (was 291 after rc2); 22 safety-marked tests across `test_human_ratings.py` + `test_exemplar_lora_build.py` + `test_v1_20_schema.py` + `test_phase3_etl.py` + `test_sidecar_pass_through.py` + `test_gen_config_overrides.py`. Restart sequence: `systemctl --user restart taco-backend`; redeploy Modal + RunPod sidecars to consume `gen_config_overrides` (pre-deploy sidecars silently ignore the new field); set `BACKEND_BEARER` on leatherjacket-dashboard process. Open operator action: tier-3 Gemma judge upstream (`gemma-4-31b-it`) is 502'ing — restart llama-swap upstream then `POST /v2/system/bulk-revalidate` to recompute scores. Training-loop progress snapshot in `docs/TRAINING_LOOP_STATUS.md`.

v1.19.0-rc1 highlights: **validator tier-2 — real Sapiens-2 pose inference (was stub since v1.17.0-rc2)**. `sapiens-sidecar/service.py` rewritten end-to-end: PyAV decode → 4 fps stride → DETR-resnet-50 person detection → `facebook/sapiens2-pose-0.4b` 308-keypoint inference on cuda:1 → aggregate `pose_temporal_stability = 1 - mean(L2 displacement) / pose_extent` (clamped [0,1]) + `pose_temporal_variance` + `human_detected` (mean conf > 0.3) + `identity_drift_frames`. Latency ~5s cold / ~2.3s warm on 81-frame 1080p; ~1.78 GB resident on cuda:1 with both Sapiens (1.5 GB) and DETR (~280 MB). Lazy-load on first request; idle eviction after `SAPIENS_IDLE_TIMEOUT_S=300` returns cuda:1 to ACE/JoyAI between bursts. New `POST /v1/admin/evict` for on-demand reclaim. Sapiens upstream package (`facebookresearch/sapiens2`, MIT/Sapiens-2 dual license) editable-installed at `/mnt/nvme-1/huggingface/sapiens2_repo`; venv rebuilt to Python 3.12 (upstream requires-python). License re-attestation in `LICENSE_NOTES.md` v1.19.0 model checkpoint section — internal-use only stands per the operator override (synthetic LTX outputs, single-tenant deploy, no external bearers active, privacy gate via `api_key_metadata.training_opt_in`). `.env` flips `LOAD_SAPIENS=1`; `config.VALIDATOR_VERSION 1.17.0-rc5 → 1.19.0-rc1` invalidates the `validator_runs` cache so the next on-complete dispatch recomputes against the real tier-2 signal. `composite()` arithmetic unchanged from rc2 — real tier2 contributes `0.2 × stability` (was `0.2 × 1.0` for stub); empirical 10-clip bulk-revalidate against rc5 historical scores: deltas -0.000 to -0.010, recommendations stable for the sampled middle-of-pack (this sample sits at 0.95-0.999 stability — middling clips outside this band will move with stronger signal). 4 new tests in `tests/test_v1_19_validator.py` (real-shape composite delta vs stub, low-stability → warn, variance fallback, [0,1] clamp); pre-existing 38 tests in `tests/test_v1_17_validator.py` still green — rc1 sidecar shape is a strict superset of rc2 stub. Restart sequence: `bash sapiens-sidecar/setup.sh` → `systemctl --user restart sapiens-sidecar` → `systemctl --user restart taco-backend`. Sidecar is auto-stopped/restored across turbo entry/exit by the existing `_stop_cuda1_tenants` / `_restore_cuda1_tenants` (LOAD_SAPIENS gating from rc2 unchanged).

v1.18.0-rc6 highlights: **shot_uuid + shot_config_key persistence — completes rc1 lineage wiring**. Latent regression in the same shape as the rc5 `_ab_arm` fix below: MCP has been forwarding `shot_uuid` + `shot_config_key` in every per-shot body since v0.7.0, but the v2 video request models (`TextToVideoRequest` / `ImageToVideoRequest` / `AudioToVideoRequest` / `RetakeRequest` / `VideoOutpaintRequest` / `VideoHdrRequest`) didn't declare them, so Pydantic v2's `extra="ignore"` was silently eating them. Net effect since rc1 (when those columns landed): every MCP-driven clip persisted NULL in `generations.shot_uuid` / `generations.shot_config_key`, breaking the lineage tables they were designed for and starving Phase C `composition_kept` pair construction of its `shot_config_key` join key (the rc3 live-DB smoke noted "no `shot_config_key` rows" — this is why). Fix: each of the 6 models gains `shot_uuid: str | None = Field(default=None, max_length=64)` + `shot_config_key: str | None = Field(default=None, max_length=128)` (no alias — MCP sends them under plain field names, unlike rc5's underscore-prefixed `_ab_arm`); every video v2 endpoint threads `body.shot_uuid` + `body.shot_config_key` into `job.params`. `_HISTORY_ONLY_PARAMS` already listed both names from rc1, so the strip path + worker_loop save call (`_params.get("shot_uuid")` → `history.save(shot_uuid=...)`) both already worked end-to-end — the only missing link was the Pydantic declaration. **No schema migration**: columns + indexes have existed since the rc1 v3 keystone. Natural follow-up to rc5: with both shipped, every MCP-driven clip now carries its full lineage (`parent_clip_id` + `shot_uuid` + `shot_config_key` + `composition_id`) plus its A/B cohort (`ab_arm`) into `generations`. 4 new tests in `tests/test_v1_19_phase_c.py`; suite now 276 green (was 270 from rc5 + 4 phase-B fixes shipped in parallel). Restart taco-backend to load the Pydantic model changes.

v1.18.0-rc5 highlights: **Stream A plumbing — `ab_arm` capture + passive-dispatch `motion_intent` + doc/script hygiene**. Closes 4 surgical gaps surfaced by the operator-control-plane audit. (a) **`ab_arm` keystone**: `history_store.CURRENT_SCHEMA_VERSION 5 → 6`; new nullable `generations.ab_arm TEXT` column (additive, idempotent). Every video v2 request model (`TextToVideoRequest`, `ImageToVideoRequest`, `AudioToVideoRequest`, `RetakeRequest`, `VideoOutpaintRequest`, `VideoHdrRequest`) gains `ab_arm: str | None = Field(default=None, alias="_ab_arm", max_length=32)` + `model_config = ConfigDict(populate_by_name=True)` — the alias accepts the underscore-prefixed wire field that MCP `_apply_ab_routing` forwards (Pydantic v2 default `extra="ignore"` was silently eating it pre-rc5). `_HISTORY_ONLY_PARAMS` extended with `"ab_arm"`; every endpoint threads `body.ab_arm` into `job.params`; `worker_loop` passes `_params.get("ab_arm")` into `history.save()`; `history.save` persists to `generations.ab_arm`. `scripts/ab_decision.py:_fetch_arm_means` switched from inferring the cohort via `lora_applied_id` to reading directly from `generations.ab_arm = ?` — column-based lookup is the load-bearing fix. **Coordinated MCP ship: noodlefinger-mcp v0.8.1** (patch-completer of v0.8.0) — `_build_clip_body` now copies `inp["_ab_arm"]` onto every per-shot HTTP body at all 4 return points (slideshow i2v / anchor a2v / is_insert / default a2v). Pre-rc5 the harness was provably incomplete: stamp existed on session input but never reached the wire. Backend rc5 + mcp v0.8.1 ship together; without v0.8.1, backend column stays NULL. (b) **`motion_intent` passive dispatch**: `_dispatch_validator(job)` now SELECTs `motion_intent` from the matching `generations` row (column has existed since rc4) and forwards to `validator.run_all_tiers(motion_intent=...)`. Every passively-validated video clip now gets the same intent reconciliation treatment as the synchronous `/v2/video/analyze-motion` path. NULL motion_intent → `None` → tier-3 prompt byte-identical to rc4 (the conditional render in `_run_tier3_judge` from rc5 already handles `None`). (c) **shot_uuid doc drift**: `docs/DECISIONS.md` ADR-014 + `docs/PRIVACY_GOVERNANCE.md` shot_uuid row updated. Both said `secrets.token_hex(16)` (random); actual code is `hashlib.sha256(prompt + image_uri + position).hexdigest()[:32]` (deterministic). Rationale captured: deterministic so the same shot across resumes hashes identically — load-bearing for rc1 lineage tables. (d) **Script hygiene**: `scripts/backfill_prompt_embeddings.py` SELECT now filters `AND g.status = 'completed'` (matches the script's stated contract — never embed in-flight or failed clips). `scripts/construct_preference_pairs.py` watermark write was already `not dry_run`-gated correctly (verified, no change). 10 new tests in `tests/test_v1_19_phase_c.py` (schema v6, ab_arm round-trip on `history.save`, Pydantic alias acceptance, ab_decision column-based cohort lookup, motion_intent fetch in `_dispatch_validator`, NULL-safety); suite now 256 green (was 246). Restart taco-backend to apply schema v6.
...
| Operator rating loop (v1.20.0) | `docs/RATING_LORA_INTEGRATION.md` + `docs/TRAINING_LOOP_STATUS.md` — `/v1/clips/{id}/rating[s]` + `/v1/ratings/queue` + `/v1/api-keys/me/validator-thresholds` + `scripts/construct_human_rating_pairs.py` |
| Exemplar set curation + LoRA build (v1.20.0) | `docs/RATING_LORA_INTEGRATION.md` — `/v1/exemplar-sets` CRUD + `POST .../build-lora` (admin, async) + `scripts/build_lora_from_exemplars.py` + `configs/exemplar_lora.yaml` |
| Stage 1 sigma + VAE tile knobs (v1.20.0) | `_get_stage1_sigmas` / `_get_decode_tiling` in `split_model_manager.py` + `POST /v1/system/config` 422 envelope + `.gen_config.history.jsonl` audit log |
...
| Integrating a third-party rating client (rating + exemplar-LoRA) | `docs/RATING_LORA_INTEGRATION.md` |
...
### sapiens pose sidecar (v1.19.0-rc1 real / v1.17.0-rc2 stub origin)
...
`systemctl stop`'d on turbo-mode entry like the other cuda:1 tenants. **As of
v1.19.0-rc1 ships REAL inference** via `facebook/sapiens2-pose-0.4b` (308
keypoints, top-down) + `facebook/detr-resnet-50` person detector. Lazy-load
on first request (~1.78 GB resident); idle eviction after
`SAPIENS_IDLE_TIMEOUT_S=300`. Output keys: `pose_temporal_stability ∈ [0,1]`,
`pose_temporal_variance`, `human_detected`, `identity_drift_frames`,
`frame_count`, `latency_s`, `stub: false`, `model_version`. The client
(`sapiens_client.py`) still tolerates the legacy `{"stub": true}` shape for
backward-compat; in rc1+ it's only emitted on the `/dev/null` smoke-test
ping path. Sample rate is `SAPIENS_SAMPLE_FPS=4` (every 6th frame on a
24fps clip — ~14 samples per 81-frame clip). `POST /v1/admin/evict`
forces on-demand VRAM reclaim without stopping the unit.
